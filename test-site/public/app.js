...
async function currentUser() {
  try { const data=await api('/api/state'); return data.user; } catch { return null; }
}
...
async function loadGrid() {
  ...
  for(const cell of data.cells){
    ...
    el.className=`booking-block ${cell.status}`;
  ...
  $("#cycleInfo").textContent=`Availability changes every ${data.cycleMs/1000} seconds. White cells remain available until reserved.`;
  return data;
}
...
async function initReservation() {
  ...
  await loadGrid();
  // Poll state so three browser sessions share the same server-side state.
...
async function initAdmin(){
  ...
  $("#openRandom").onclick=()=>action('open_random');
  $("#clearLog").onclick=()=>action('clear_log');
  $("#openCell").onclick=()=>action('open_cell',Number($("#cellNumber").value));
  $("#closeCell").onclick=()=>action('close_cell',Number($("#cellNumber").value));
